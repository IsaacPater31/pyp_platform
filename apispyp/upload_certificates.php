...
if (!isset($_FILES['certificado'])) {
    $errors['certificado'] = 'Archivo PDF requerido';
}
if (!empty($errors)) {
...
$pdf = $_FILES['certificado'];

// Solo PDF permitido
$extension = strtolower(pathinfo($pdf['name'], PATHINFO_EXTENSION));
if ($extension !== 'pdf') {
    $response['errors']['certificado'] = 'Solo se permiten archivos PDF';
...
$carpeta = $_SERVER['DOCUMENT_ROOT'] . '/pyp_platform/certificates/';
if (!file_exists($carpeta)) {
    mkdir($carpeta, 0777, true);
}

// Nombre único
$nombreArchivo = 'certificados_' . $id_profesional . '_' . time() . '.pdf';
...
try {
    $stmt = $conn->prepare("UPDATE profesionales SET certificados_especialidades = ? WHERE id = ?");
    $stmt->bind_param('si', $nombreArchivo, $id_profesional);

    if ($stmt->execute()) {
        $response['success'] = true;
        $response['message'] = 'Certificados PDF subido correctamente';
        $response['nombreArchivo'] = $nombreArchivo;

        // URL pública del archivo
        $protocolo = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? "https" : "http";
        $host = $_SERVER['HTTP_HOST'];
        $rutaPublica = "/pyp_platform/certificates/" . $nombreArchivo;
        $response['url_certificado'] = $protocolo . "://" . $host . $rutaPublica;
    ...
    error_log('ERROR en subida de certificado profesional: ' . $e->getMessage());
