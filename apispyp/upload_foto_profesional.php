...
require 'connection.php'; // Conexión igual que tu otro API
...
$carpeta = $_SERVER['DOCUMENT_ROOT'] . '/pyp_platform/profile_pictures/';
if (!file_exists($carpeta)) {
    mkdir($carpeta, 0777, true);
}

// Nombre único
$nombreArchivo = 'profesional_' . $id_profesional . '_' . time() . '.' . $extension;
...
try {
    $stmt = $conn->prepare("UPDATE profesionales SET foto_perfil = ? WHERE id = ?");
    ...
    if ($stmt->execute()) {
        ...
        $rutaPublica = "/pyp_platform/profile_pictures/" . $nombreArchivo;
    ...
    error_log('ERROR en subida de foto profesional: ' . $e->getMessage());
