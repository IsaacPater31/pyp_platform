...
$query = "SELECT id, password, estado_validacion FROM profesionales WHERE username = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("s", $username);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($id, $hashedPassword, $estadoValidacion);
...
if ($stmt->num_rows > 0) {
    ...
    if ($estadoValidacion === 'pendiente') {
        $response['message'] = 'Tu cuenta está pendiente de validación. No puedes iniciar sesión aún.';
        http_response_code(403); // Forbidden
    } elseif ($estadoValidacion === 'rechazado') {
        $response['message'] = 'Tu cuenta ha sido rechazada. No puedes iniciar sesión.';
        http_response_code(403); // Forbidden
